...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function bindPrecisionEvents(){
    els.tileLayer.querySelectorAll(".precision-object-group").forEach(group=>{group.addEventListener("pointerdown",e=>e.stopPropagation());const main=group.querySelector("[data-precision-main]");main?.addEventListener("click",e=>{e.stopPropagation();const key=group.dataset.precisionGroup,items=ensureObjectIndexes().anchorGroupByKey.get(key)?.items||[];if(items.length>1){state.precisionClusterOpen=state.precisionClusterOpen===key?null:key;scheduleRender()}else if(main.dataset.precisionMain)selectObject(main.dataset.precisionMain)});main?.addEventListener("dblclick",e=>{e.stopPropagation();openObjectForm(indexedObject(main.dataset.precisionMain))});main?.addEventListener("mouseenter",e=>{const o=indexedObject(main.dataset.precisionMain);if(o)showTooltip(`${o.name} · ${o.type||'未分类'} · ${coordText(o.x,o.y)}`,e.clientX,e.clientY)});main?.addEventListener("mousemove",e=>moveTooltip(e.clientX,e.clientY));main?.addEventListener("mouseleave",hideTooltip);
  ...
  function animateCameraTo(x,y,zoom,duration=680,done=null){
    if(typeof duration==="function"){done=duration;duration=680}if(state.cameraAnimation)cancelAnimationFrame(state.cameraAnimation);
    const from={x:state.camera.x,y:state.camera.y,z:state.camera.zoom},start=performance.now(),light=zoomRenderBand(from.z)===zoomRenderBand(zoom),ease=t=>1-Math.pow(1-t,3);
    const step=now=>{const t=Math.min(1,(now-start)/duration),e=ease(t);state.camera.x=from.x+(x-from.x)*e;state.camera.y=from.y+(y-from.y)*e;state.camera.zoom=from.z+(zoom-from.z)*e;if(light){applyDeferredLayerTransform({x:from.x,y:from.y,zoom:from.z});scheduleCameraFrame()}else scheduleRender();if(t<1)state.cameraAnimation=requestAnimationFrame(step);else{state.cameraAnimation=null;resetDeferredLayerTransform();scheduleRender();done&&done()}};state.cameraAnimation=requestAnimationFrame(step)
  }
  ...
  function setZoom(newZoom,anchorClient=null,deferDOM=false){
    const oldCamera={x:state.camera.x,y:state.camera.y,zoom:state.camera.zoom},z=Math.max(MIN_ZOOM,Math.min(MAX_ZOOM,newZoom));if(anchorClient){const before=screenToWorld(anchorClient.x,anchorClient.y);state.camera.zoom=z;const after=screenToWorld(anchorClient.x,anchorClient.y);state.camera.x+=before.x-after.x;state.camera.y+=before.y-after.y}else state.camera.zoom=z;updateHoverLensSuppression();
    if(deferDOM&&zoomRenderBand(oldCamera.zoom)===zoomRenderBand(state.camera.zoom)){if(!state.perf.wheelBase)state.perf.wheelBase=oldCamera;applyDeferredLayerTransform(state.perf.wheelBase);scheduleCameraFrame();clearTimeout(state.perf.wheelSettleTimer);state.perf.wheelSettleTimer=setTimeout(()=>{resetDeferredLayerTransform();scheduleRender();persist()},90)}else{resetDeferredLayerTransform();scheduleRender();persist()}
  }
