...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  const migratedWorkspaceObjects=migrateWorkspaceObjects(saved);
  const state = {
  ...
  function v052Hierarchy(){
    ...
    const bounds=ensureObjectIndexes().bounds||{minX:0,maxX:0,minY:0,maxY:0},world={...(HIERARCHY_SEED.world||{}),id:HIERARCHY_SEED.world?.id||"world-shanhaijing",name:HIERARCHY_SEED.world?.name||"山海经世界",type:"world",origin:HIERARCHY_SEED.world?.origin||{name:"都广之野",x:0,y:0},unit:"里",mainGridLi:CELL_LI,innerGridLi:10,childRegionIds:macroRegions.map(r=>r.id),bounds,objectCount:state.objects.length,status:"active",environmentProfile:null};
    const regions=[...macroRegions,...detailRegions],regionById=new Map(regions.map(r=>[r.id,r])),hierarchy={schemaVersion:HIERARCHY_SEED.schemaVersion||"world-region-place-1.0",world,regions,unassignedRegionId:unassignedId,stats:{macroRegionCount:macroRegions.length,regionCount:detailRegions.length,assignedObjectCount:state.objects.filter(o=>o.primaryRegionId!==unassignedId).length,unassignedObjectCount:state.objects.filter(o=>o.primaryRegionId===unassignedId).length}};
  ...
  function getSearchContext(){
    ...
    let result;
    if(!nq)result={q:"",objectMatches,tileMatches,directCells,exactNameCells,nameCells,areaIds,relatedCells,nameMatches:[],contentMatches:[],filteredObjects:state.objects.filter(objectPassesSelectFilters)};
    else{
  ...
  function ensureObjectIndexes(){
    ...
    state.objects.forEach(o=>{
      ...
      const a=objectAnchor(o),ak=`${a.x.toFixed(1)},${a.y.toFixed(1)}`;if(!anchorGroupByKey.has(ak))anchorGroupByKey.set(ak,{key:ak,x:a.x,y:a.y,items:[]});anchorGroupByKey.get(ak).items.push(o);
      xs.push(a.x);ys.push(a.y);
      if(o.area){areas.push(o);const b=rangeBounds(o.area);xs.push(b.west,b.east);ys.push(b.south,b.north)}
      if(o.geometryType==="line"&&o.path?.length>=2){lines.push(o);o.path.forEach(pt=>{xs.push(Number(pt[0])||0);ys.push(Number(pt[1])||0)})}
    });
  ...
  function buildCellMap(objects=state.objects){if(objects===state.objects)return ensureObjectIndexes().cellMap;const m=new Map();objects.forEach(o=>{const {gx,gy}=objectCell(o),k=cellKey(gx,gy);if(!m.has(k))m.set(k,[]);m.get(k).push(o)});m.forEach((items,key)=>m.set(key,sortObjects(items)));return m}
  ...
  function updateHeader(){
    ...
    els.versionLine.textContent=`地图数据 ${state.dataVersion} · 规格 ${meta.specProduction||"v004"} · ${state.objects.length}个对象 · ${state.waterPaths.length}段水系 · 100里主格 / 10里格内精度`;
  ...
  function renderSidebar(){
    ...
    if(q){
      ...
      els.objectList.innerHTML=items.slice(0,260).map(o=>{const status=objectIndexStatus(o);return `<button class="object-list-item ${o.id===state.selectedId?'selected':''}" data-object-id="${esc(o.id)}"><strong><em>${esc(o.rowRef||'NEW')}</em>${esc(o.name)}</strong><small>${esc(o.type)} · ${esc(o.chapter||'未标经篇')} · ${coordText(o.x,o.y)}</small>${status?`<span class="object-index-status ${status.kind}" title="${esc(status.title)}">${esc(status.label)}</span>`:""}</button>`}).join("")||`<div style="padding:20px 8px;color:var(--muted);font-size:10px">没有匹配对象。</div>`;
...
完整的地块或对象描述。`;
  ...
  function renderV029Minimap(force=false){
    ...
    const redraw=force||state.perf.minimapRevision!==state.objectRevision||state.perf.minimapSize!==sizeKey;if(redraw){const idx=ensureObjectIndexes(),{minX,maxX,minY,maxY}=idx.bounds,pad=12,sx=(w-pad*2)/(maxX-minX||1),sy=(h-pad*2)/(maxY-minY||1),sc=Math.min(sx,sy),ox=(w-(maxX-minX)*sc)/2,oy=(h-(maxY-minY)*sc)/2,pt=(x,y)=>({x:ox+(x-minX)*sc,y:h-(oy+(y-minY)*sc)});canvas.width=w*dpr;canvas.height=h*dpr;const ctx=canvas.getContext("2d");ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);ctx.fillStyle="#ece8dc";ctx.fillRect(0,0,w,h);idx.lines.forEach(o=>{ctx.beginPath();o.path.forEach((p,i)=>{const q=pt(p[0],p[1]);i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y)});ctx.strokeStyle=isHydrologyObject(o)?"rgba(43,127,153,.58)":"rgba(92,85,72,.38)";ctx.lineWidth=1;ctx.stroke()});state.objects.forEach(o=>{const a=objectAnchor(o),q=pt(a.x,a.y),pal=TERRAIN_PALETTE[isHydrologyObject(o)?"water":terrainCategory(o)]||TERRAIN_PALETTE.unknown;ctx.fillStyle=pal.color;ctx.globalAlpha=.7;ctx.fillRect(q.x-1,q.y-1,2,2)});ctx.globalAlpha=1;const z=pt(0,0);ctx.fillStyle="#ad4d3d";ctx.fillRect(z.x-2,z.y-2,4,4);wrap.dataset.bounds=JSON.stringify({minX,maxX,minY,maxY,sc,ox,oy,w,h});state.perf.minimapRevision=state.objectRevision;state.perf.minimapSize=sizeKey}updateV029MinimapViewport()
  }
  ...
  function findV029IsolatedObjects(){
    if(state.perf.isolatedRevision===state.objectRevision)return state.perf.isolated;const idx=ensureObjectIndexes(),objects=state.objects,texts=objects.map(o=>normalizeText(v029ObjectText(o)));
