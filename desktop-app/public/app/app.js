...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function recordChange(c){const cellEntity=String(c.entityId||"").startsWith("CELL-")||c.entityType==="tile_profile",now=new Date(),timestamp=now.toISOString();if(cellEntity){const key=String(c.entityId||"").replace(/^CELL-/,"");if(state.tileProfiles[key])state.tileProfiles[key].updatedAt=timestamp}else if(c.operation!=="delete"){const target=state.objects.find(o=>o.id===c.entityId);if(target)target.updatedAt=timestamp}if(!cellEntity||c.before?.objects||c.after?.objects)invalidateObjectCaches();if(cellEntity)invalidateProfileCaches();state.changes.push({...c,changeId:`CHG-${Date.now()}-${Math.random().toString(36).slice(2,6)}`,timestamp,time:now.toLocaleString("zh-CN"),baseVersion:state.dataVersion});updateHeader()}
...
完整的地块或对象描述。`;
  ...
  function v029RelationContext(){
    ...
    const catalog=v045RelationCatalog(selected),preset=state.viewPreset||"all",limit=preset==="all"?36:28;
    const scored=catalog.filter(x=>v044RelationPresetMatch(selected,x.o,x.info,x)).slice(0,limit);
  ...
  function drawV029RelationLabel(ctx,area,hovered,bounds,occupied=[]){
    const p=v029QuadraticPoint(area.p0,area.cp,area.p1,area.labelT||.58),target=shortText(area.targetName||"关联对象",14),relation=shortText(area.info?.label||"资料关联",14),targetSize=hovered?11:10,relationSize=hovered?9:8;
    ctx.save();ctx.font=`800 ${targetSize}px sans-serif`;const targetW=ctx.measureText(target).width;ctx.font=`700 ${relationSize}px sans-serif`;const relationW=ctx.measureText(relation).width;const w=Math.min(220,Math.max(92,Math.max(targetW,relationW)+22)),h=hovered?40:36;
