...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function drawCanvas(light=false){
    ...
    if(state.precisionMode){drawPrecisionTerrain(ctx,v,s);drawPrecisionGrid(ctx,v,s);drawV044LocationWatermark(ctx,r);if(state.layers.rivers)drawPrecisionLines(ctx,v,s)}
  ...
  function selectObject(id){state.spatialFocusArmed=true;state.selectedId=id;const o=indexedObject(id);if(o){const c=objectCell(o);state.selectedCell=cellKey(c.gx,c.gy)}renderDetails();renderSidebar();scheduleRender();persist()}
...
完整的地块或对象描述。`;
  ...
  function bindEvents(){
    ...
    document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(visibleElement("identityTagExplorer")){closeTagExplorer();return}if(!els.dossierWorkspace.classList.contains("hidden")){closeDossierWorkspace();return}if(!els.rangeWorkspace.classList.contains("hidden")){closeRangeEditor();return}if(!els.importWorkspace.classList.contains("hidden")){closeImportWorkspace();return}$$(".modal:not(.hidden)").forEach(m=>m.classList.add("hidden"));state.flippedCell=null;scheduleRender()}if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="s"){e.preventDefault();if(!els.rangeWorkspace.classList.contains("hidden"))saveRangeEditor();else exportPatch(false)}})
  }
