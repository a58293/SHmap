...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function renderRoundSummary(meta){if(!meta)return;const status=meta.uploadStatus||"pending",stateText=status==="uploading"?"正在提交更改包与图片资源到 GitHub，请不要关闭程序。":status==="success"?`已上传到 ${meta.remotePath||GITHUB_CONFIG.pendingPath}，其他设备可检查数据更新。`:status==="error"?`上传失败：${meta.uploadError||"未知错误"}`:status==="exported"?"已导出可携带图片的本地更改包，但尚未上传 GitHub。":"等待上传 GitHub。";els.roundSummary.innerHTML=`<strong>${esc(meta.changeCount)}项更改</strong><br>${esc(meta.summary)}<br><span>同步图片：${Number(meta.assetCount)||0}张</span><br><span>文件：${esc(meta.filename)}</span><br><span>基础数据：${esc(meta.baseVersion)} · ${esc(meta.createdLabel)}</span><div class="round-publish-state ${status}">${esc(stateText)}</div>`;if(els.roundModalLead)els.roundModalLead.textContent=status==="uploading"?"正在自动上传本轮更改包与图片资源。上传成功后会自动归档并开始下一轮。":status==="success"?"本轮资料和图片已上传，正在归档并开始下一轮。":status==="error"?"自动上传没有完成。可以修复网络后重试，或先导出到本地。":"本轮更改包与图片清单已生成。";if(els.roundNote)els.roundNote.textContent=status==="success"?"其他设备启动后会自动检查；图片按指纹去重并从资源目录加载。":status==="error"?"本轮记录尚未清空；只有上传成功或本地导出后才允许归档。":"归档不会删除地图数据。";const busy=status==="uploading";els.roundKeepBtn.disabled=busy;els.roundArchiveBtn.disabled=busy||status==="error"||status==="pending";els.roundRetryPublishBtn?.classList.toggle("hidden",status!=="error");els.roundExportFallbackBtn?.classList.toggle("hidden",status!=="error");if(status==="exported")els.roundArchiveBtn.disabled=false}
...
完整的地块或对象描述。`;
  ...
  function renderTileObjectEditor(){
    ...
    if(!items.length){els.tileObjectEditorList.innerHTML=`<div class="tile-object-editor-empty">本格尚无对象。<br>点击“新增”建立第一个对象。</div>`;return}
  ...
  function objectImageSource(o){return syncedImageUrl(String(o?.imageUrl||o?.image||"").trim())}
  ...
  function briefMuseumObjectHTML(o,cat){
    const imported=!!o.importedFromDossier,source=cardScriptureSource(o),directOriginal=imported&&isDirectOriginalRelation(o.localRelation)&&hasText(o.originalExcerpt),relationRow=directOriginal?["原文",o.originalExcerpt]:["与本地关系",displayLocalRelation(o.localRelation)],rows=[
      ...
      ["功效",o.efficacy],
      ["备注",o.notes]
    ].filter(([,v])=>hasText(v)&&!/原文未载明确/.test(String(v))).slice(0,5),src=objectImageSource(o),objectDetail=o.id?` data-object-detail="${esc(o.id)}"`:"",dossierDetail=!o.id&&o.dossierOwnerId?` data-dossier-detail-owner="${esc(o.dossierOwnerId)}" data-dossier-detail-index="${Number(o.dossierIndex)||0}"`:"",detail=objectDetail||dossierDetail,tab=detail?' tabindex="0"':"",hasImportedDetails=imported&&rows.length;
  ...
  function renderIdentityBoard(profile,items,tile,main,{prefix="dossier"}={}){
    ...
    const tileImage=tileProfileImageHTML(profile,name),heroSide=`<div class="identity-hero-side">${tileImage}<div class="identity-hero-actions"><button data-board-copy="${esc(tile.key)}">复制本页摘要</button><button data-board-full="${esc(tile.key)}">打开完整博物志</button></div></div>`;
  ...
  function renderDossierContent(profile,items,tile,main,b){
    ...
    const childHierarchyHtml=objectChildHierarchyPanelHTML(main?[main]:items),tileImage=tileProfileImageHTML(profile,main?.name||profile.tileType||"地块","full");els.dossierContent.innerHTML=dossierTopicSwitcherHTML(items,main)+tileImage+(html||`<div class="dossier-empty">本栏尚未录入。</div>`)+childHierarchyHtml
  }
  ...
  function mergeNormalizedDossierEntry(target,source){
    const merged={...target};["localRelation","coreFeatures","evidence","efficacy","notes"].forEach(field=>{merged[field]=mergeSupplementText(merged[field],source?.[field])});merged.imageUrl=merged.imageUrl||source?.imageUrl||"";merged.linkedObjectId=merged.linkedObjectId||source?.linkedObjectId||"";merged.linkReason=merged.linkReason||source?.linkReason||"";return merged
  }
