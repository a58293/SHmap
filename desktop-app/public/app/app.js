...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function drawCanvas(light=false){
    ...
    if(!state.regionOverviewMode)drawBrushSelection(ctx,v);if(!light){drawContextDimming(ctx,v);drawSearchHighlights(ctx,v,s);drawV029RelationOverlay(ctx,v,s);drawV029MeasureOverlay(ctx,v,s)}drawOrigin(ctx,s);
  }
...
完整的地块或对象描述。`;
  ...
  function briefMuseumObjectHTML(o,cat){const src=objectImageSource(o),rows=[["与本地关系",o.localRelation],["核心特征",o.coreFeatures||objectCoreText(o)],["功效",o.efficacy],["证据",evidenceTextForObject(o)]].filter(([,v])=>hasText(v)).slice(0,4);return `<article class="brief-museum-object" data-object-detail="${esc(o.id)}"><div class="brief-object-image ${src?'has-image':'is-placeholder'}">${src?`<img src="${esc(src)}" alt="${esc(o.name)}" loading="lazy" decoding="async">`:""}<div class="brief-image-placeholder"><i>${cat.glyph}</i><span>待补图</span></div></div><div class="brief-object-copy"><div class="brief-object-title"><strong>${esc(o.name)}</strong><small>${esc(o.type||cat.label)}</small></div>${rows.length?`<div class="brief-object-rows">${rows.map(([k,v])=>`<div><b>${esc(k)}</b><span>${esc(shortText(v,100))}</span></div>`).join("")}</div>`:`<p class="brief-object-empty">资料待补充</p>`}</div></article>`}
  function briefMuseumHTML(items){const categories=V027_CATEGORIES.map(cat=>({...cat,objects:items.filter(o=>objectCategory(o)===cat.key)})).filter(cat=>cat.objects.length);if(!categories.length)return `<section class="brief-museum empty"><div class="brief-museum-head"><div><span class="eyebrow">OBJECT CABINET</span><h3>这里有什么</h3></div></div><div class="brief-museum-empty"><i>录</i><span>本地块尚无可陈列的独立对象。</span></div></section>`;return `<section class="brief-museum"><div class="brief-museum-head"><div><span class="eyebrow">OBJECT CABINET</span><h3>这里有什么</h3></div><p>${categories.length} 类 · ${items.length} 个对象；图片可后续逐项补充。</p></div><div class="brief-museum-grid count-${categories.length}" style="--museum-cols:${Math.min(categories.length,6)}">${categories.map(cat=>`<article class="brief-museum-category accent-border-${cat.key}"><header><div><i>${cat.glyph}</i><strong>${cat.label}</strong></div><b>${cat.objects.length}</b></header><div class="brief-museum-list">${cat.objects.map(o=>briefMuseumObjectHTML(o,cat)).join("")}</div></article>`).join("")}</div></section>`}
  ...
  function renderBrushCollectionInDossier(){
    ...
    els.dossierContent.querySelector('[data-collection-clear]')?.addEventListener('click',()=>{state.brushKeys.clear();clearBrushTraces();updateBrushUI();persist();closeDossierWorkspace();toast("已清空地块集合","可以重新使用画笔采集。")});
  ...
  function startBrushTrace(clientX,clientY){
    if(state.brushErase)return;const w=screenToWorld(clientX,clientY),stroke=[{x:w.x,y:w.y}];state.brushCurrentStroke=stroke;state.brushStrokes.push(stroke);
    if(state.brushStrokes.length>40)state.brushStrokes.splice(0,state.brushStrokes.length-40);scheduleBrushTraceDraw()
  }
  function finishBrushTrace(){const stroke=state.brushCurrentStroke;if(stroke?.length>2){const simplified=simplifyBrushStroke(stroke,Math.max(.8,2.2/Math.max(scale(),.001)));stroke.splice(0,stroke.length,...simplified)}state.brushCurrentStroke=null;scheduleBrushTraceDraw()}
  ...
  function drawBrushTraceCanvas(){
    ...
    const strokes=state.brushStrokes||[];if(!strokes.length)return;ctx.save();ctx.lineCap="round";ctx.lineJoin="round";
  ...
  function updateBrushUI(render=true){const btn=document.getElementById("brushModeBtn"),collection=document.getElementById("brushCollectionBtn"),count=document.getElementById("brushCollectionCount");btn?.classList.toggle("active",state.brushMode&&!state.brushErase);btn?.classList.toggle("erase",state.brushMode&&state.brushErase);if(btn)btn.textContent=state.brushMode?(state.brushErase?"⌁ 擦除地块":"⌁ 画笔采集中"):"⌁ 画笔采集";if(count)count.textContent=state.brushKeys.size;if(collection)collection.classList.toggle("hidden",!state.brushKeys.size);els.viewport.classList.toggle("brush-mode",state.brushMode);if(render)scheduleRender()}
  ...
  function setupV027Features(){
    ...
    const up=e=>{if(!state.brushMode||!state.brushDrawing)return;e.preventDefault();e.stopImmediatePropagation();if(!state.brushErase)brushTracePoint(e.clientX,e.clientY);state.brushDrawing=false;finishBrushTrace();try{els.viewport.releasePointerCapture(e.pointerId)}catch{}persist();if(!state.brushErase&&state.brushKeys.size)openBrushCollection()};
    ...
    els.viewport.addEventListener("contextmenu",e=>{if(e.target.closest("button,input,select,textarea"))return;const w=screenToWorld(e.clientX,e.clientY),key=cellKey(cellIndex(w.x),cellIndex(w.y));if(!state.brushKeys.has(key))return;e.preventDefault();e.stopImmediatePropagation();state.brushKeys.delete(key);if(!state.brushKeys.size)clearBrushTraces();updateBrushUI();persist();toast("已取消选择",`地块 ${key} 已从画笔集合移除`);if(state.dossierCollectionMode){if(state.brushKeys.size)renderBrushCollectionInDossier();else closeDossierWorkspace()}},true);
