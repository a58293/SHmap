...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  const state = {
    ...
    layers:{areas:true,terrain:true,rivers:true,empty:false,changes:true},
    precisionMode:false,
  ...
  function init(){
    cleanupExpiredTrash(false);ensureObjectIndexes();populateFilters();bindEvents();renderSidebar();renderDetails();resizeCanvas();scheduleRender();updateHeader();renderExamplePanel();
...
完整的地块或对象描述。`;
  ...
  function briefMuseumHTML(items){
    const imported=importedMuseumObjects(items),source=imported||items,categories=V027_CATEGORIES.map(cat=>({...cat,objects:source.filter(o=>objectCategory(o)===cat.key)})).filter(cat=>cat.objects.length),total=source.length;
    if(!categories.length)return `<section class="brief-museum empty"><div class="brief-museum-head"><div><span class="eyebrow">OBJECT CABINET</span><h3>这里有什么</h3></div></div><div class="brief-museum-empty"><i>录</i><span>本地块尚无可陈列的独立对象。</span></div></section>`;
    return `<section class="brief-museum"><div class="brief-museum-head"><div><span class="eyebrow">OBJECT CABINET</span><h3>这里有什么</h3></div><p>${categories.length} 类 · ${total} 个条目；图片可后续逐项补充。</p></div><div class="brief-museum-grid count-${categories.length}" style="--museum-cols:${Math.min(categories.length,6)}">${categories.map(cat=>`<article class="brief-museum-category accent-border-${cat.key}"><header><div><i>${cat.glyph}</i><strong>${cat.label}</strong></div><b>${cat.objects.length}</b></header><div class="brief-museum-list">${cat.objects.map(o=>briefMuseumObjectHTML(o,cat)).join("")}</div></article>`).join("")}</div></section>`
  }
  function renderImportAnalysis(a){
  ...
  function renderBrushCollectionInDossier(){
    ...
    els.dossierContent.querySelector('[data-collection-clear]')?.addEventListener('click',clearBrushCollectionGesture);
    ...
    els.dossierContent.querySelectorAll('[data-collection-remove]').forEach(b=>b.addEventListener('click',()=>{state.brushKeys.delete(b.dataset.collectionRemove);if(!state.brushKeys.size)clearBrushTraces();updateBrushUI();persist();if(state.brushKeys.size)renderBrushCollectionInDossier();else closeDossierWorkspace()}));
  }
  ...
  function updateBrushUI(render=true){
    const btn=document.getElementById("brushModeBtn"),cancel=document.getElementById("cancelBrushBtn"),collection=document.getElementById("brushCollectionBtn"),count=document.getElementById("brushCollectionCount");
  ...
  function handleBrushRightClick(e){
    if(!state.brushMode)return false;e.preventDefault();e.stopImmediatePropagation();
    const now=Date.now();
    if(state.brushRightClickTimer&&now-state.brushRightClickAt<=420){clearTimeout(state.brushRightClickTimer);state.brushRightClickTimer=null;state.brushRightClickAt=0;clearBrushCollectionGesture();return true}
    state.brushRightClickAt=now;state.brushRightClickTimer=setTimeout(()=>{state.brushRightClickTimer=null;state.brushRightClickAt=0;undoLastBrushAction()},360);return true
  }
  ...
  function setupV027Features(){
    const brushBtn=document.getElementById("brushModeBtn"),cancelBrushBtn=document.getElementById("cancelBrushBtn"),collectionBtn=document.getElementById("brushCollectionBtn"),collection=document.getElementById("brushCollectionWorkspace");
    brushBtn?.addEventListener("click",()=>setBrushMode(!state.brushMode,false));cancelBrushBtn?.addEventListener("click",cancelBrushMode);collectionBtn?.addEventListener("click",openBrushCollection);
