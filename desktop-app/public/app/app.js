...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function createPatchMeta(){if(!state.changes.length){toast("没有可提交的更改","先新增、修改或删除内容后再完成本轮。","error");return null}const {now,filename}=patchFileMeta(),snapshot=cloneJSON(state.changes),pkg={package_type:"shjpatch",package_version:"1.2",base_data_version:state.dataVersion,created_at:now.toISOString(),change_count:snapshot.length,summary:buildChangeSummary(snapshot),changes:snapshot};return {filename,createdAt:now.toISOString(),createdLabel:now.toLocaleString("zh-CN"),baseVersion:state.dataVersion,summary:pkg.summary,changeCount:snapshot.length,changes:snapshot,payload:JSON.stringify(pkg,null,2),uploadStatus:"pending",uploadError:"",uploaded:false,exported:false}}
  function exportPatch(openFinishDialog=false){const meta=createPatchMeta();if(!meta)return null;download(meta.filename,meta.payload);meta.exported=true;meta.uploadStatus="exported";state.pendingRoundExport=meta;toast("更改包已导出",`${meta.changeCount}项更改 · ${meta.filename}`);if(openFinishDialog){renderRoundSummary(meta);openModal("roundModal")}return meta}
  function renderRoundSummary(meta){if(!meta)return;const status=meta.uploadStatus||"pending",stateText=status==="uploading"?"正在提交到 GitHub，请不要关闭程序。":status==="success"?`已上传到 ${meta.remotePath||GITHUB_CONFIG.pendingPath}，其他设备可检查数据更新。`:status==="error"?`上传失败：${meta.uploadError||"未知错误"}`:status==="exported"?"已导出到本地，但尚未上传 GitHub。":"等待上传 GitHub。";els.roundSummary.innerHTML=`<strong>${esc(meta.changeCount)}项更改</strong><br>${esc(meta.summary)}<br><span>文件：${esc(meta.filename)}</span><br><span>基础数据：${esc(meta.baseVersion)} · ${esc(meta.createdLabel)}</span><div class="round-publish-state ${status}">${esc(stateText)}</div>`;if(els.roundModalLead)els.roundModalLead.textContent=status==="uploading"?"正在自动上传本轮更改包。上传成功后会自动归档并开始下一轮。":status==="success"?"本轮已上传，正在归档并开始下一轮。":status==="error"?"自动上传没有完成。可以修复网络后重试，或先导出到本地。":"本轮更改包已生成。";if(els.roundNote)els.roundNote.textContent=status==="success"?"其他设备启动后会自动检查，也可以手动点击“检查数据更新”。":status==="error"?"本轮记录尚未清空；只有上传成功或本地导出后才允许归档。":"归档不会删除地图数据。";const busy=status==="uploading";els.roundKeepBtn.disabled=busy;els.roundArchiveBtn.disabled=busy||status==="error"||status==="pending";els.roundRetryPublishBtn?.classList.toggle("hidden",status!=="error");els.roundExportFallbackBtn?.classList.toggle("hidden",status!=="error");if(status==="exported")els.roundArchiveBtn.disabled=false}
  async function publishPendingRound(){const meta=state.pendingRoundExport;if(!meta||meta.uploadStatus==="uploading")return;if(!window.SHJ_DESKTOP?.active||typeof window.SHJ_DESKTOP.publishPatch!=="function"){meta.uploadStatus="error";meta.uploadError="当前不是桌面程序，无法自动调用本机 Git 与 GitHub Desktop。";renderRoundSummary(meta);return}meta.uploadStatus="uploading";meta.uploadError="";renderRoundSummary(meta);let repoPath=localStorage.getItem(PUBLISH_REPO_KEY)||"";const invokePublish=path=>window.SHJ_DESKTOP.publishPatch({repoPath:path||null,fileName:meta.filename,content:meta.payload,commitMessage:`data: 发布 ${meta.filename}`});try{let result;try{result=await invokePublish(repoPath)}catch(error){const message=String(error||"");if(message.includes("PUBLISH_REPO_REQUIRED")){const chosen=window.prompt("未找到本地 SHmap 仓库。请输入仓库根目录：",repoPath||"F:\\SHmap\\SHmap");if(!chosen)throw new Error("未选择本地仓库目录");repoPath=chosen.trim();result=await invokePublish(repoPath)}else throw error}meta.uploadStatus="success";meta.uploaded=true;meta.repoPath=result?.repoPath||repoPath;meta.remotePath=result?.remotePath||`${GITHUB_CONFIG.pendingPath}/${meta.filename}`;meta.commit=result?.commit||"";meta.uploadedAt=result?.pushedAt||new Date().toISOString();if(meta.repoPath)localStorage.setItem(PUBLISH_REPO_KEY,meta.repoPath);renderRoundSummary(meta);toast("本轮已上传 GitHub",`${meta.changeCount}项更改 · 其他设备可检查数据更新`);setTimeout(()=>{if(state.pendingRoundExport===meta&&meta.uploadStatus==="success")archiveCurrentRound(true)},650)}catch(error){meta.uploadStatus="error";meta.uploadError=String(error?.message||error||"上传失败").replace(/^Error:\s*/,"");renderRoundSummary(meta);toast("自动上传失败",meta.uploadError,"error")}}
  async function finishRoundAndPublish(){if(!state.changes.length){toast("没有可提交的更改","先新增、修改或删除内容后再完成本轮。","error");return}persist(true);try{await window.SHJ_DESKTOP?.flush?.()}catch(error){toast("工作区保存失败",String(error||"无法写入桌面数据库"),"error");return}state.pendingRoundExport=createPatchMeta();if(!state.pendingRoundExport)return;openModal("roundModal");renderRoundSummary(state.pendingRoundExport);await publishPendingRound()}
...
完整的地块或对象描述。`;
  ...
  function importedMuseumObjects(items){
    ...
    const ownerNames=new Set([main?.name,...splitTags(main?.aliases),...splitTags(main?.sameName)].map(name=>normalizeText(normalizeDossierEntryName(name))).filter(Boolean));
    const resolvedEntries=visibleEntries.map(({entry,index})=>({entry,index,linked:resolvedDossierEntryObject(entry,main)})).filter(({entry,linked})=>{
      ...
      return !entryName||!ownerNames.has(entryName)
    });
  ...
  function syncTileImageManager(){
    const input=document.getElementById("tileImageUrl"),preview=document.getElementById("tileImagePreview"),src=String(input?.value||"").trim();if(!preview)return;
  ...
  function objectImageSource(o){return String(o?.imageUrl||o?.image||"").trim()}
  ...
  function tileProfileImageHTML(profile,name,mode="hero"){
    const src=String(profile?.imageUrl||"").trim();if(!src)return "";const source=profile?.imageSource||"地块主体图片",copyright=profile?.imageCopyright||"";
  ...
  async function v060ImageFileToDataUrl(file){if(!file)return null;const bitmap=await createImageBitmap(file),max=1600,scale=Math.min(1,max/Math.max(bitmap.width,bitmap.height)),canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));canvas.getContext("2d",{alpha:false}).drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close?.();const blob=await new Promise(resolve=>canvas.toBlob(resolve,"image/webp",.86));if(!blob)throw new Error("无法生成WebP图片");return {dataUrl:canvas.toDataURL("image/webp",.86),bytes:[...new Uint8Array(await blob.arrayBuffer())],extension:"webp"}}
  ...
  window.__SHJ_APP_RUNTIME_INFO__={version:"0.9.3",renderArchitecture:"single-static-runtime",objectRoleSchema:"entity-collection-subregion-path-detail-context-1.0",relationRendering:"edge-routed-clickable-explained-bundled",environmentRendering:"data-derived-static-overview-fade-to-tile-cards",visualTheme:"yujian-shanhai-assets",scriptureDirectory:"eighteen-full-content-pages",waterDisplay:"special-model-audited-dossier-and-render-tiers",bootGuard:true};
  setupV027State();
  init();
