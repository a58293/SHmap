...
完整的地块或对象描述。`;
  ...
  function openTileProfileForm(){
    ...
    Object.entries(map).forEach(([id,key])=>setFieldValue(id,profile[key]));renderScriptureEditors(profile);renderTileObjectEditor();openModal("tileProfileModal")
  }
  ...
  function renderDossierContent(profile,items,tile,main,b){
    ...
    const childHierarchyHtml=objectChildHierarchyPanelHTML(main?[main]:items);els.dossierContent.innerHTML=dossierTopicSwitcherHTML(items,main)+(html||`<div class="dossier-empty">本栏尚未录入。</div>`)+childHierarchyHtml
  }
  ...
  window.__SHJ_APP_RUNTIME_INFO__={version:"0.9.2",renderArchitecture:"single-static-runtime",objectRoleSchema:"entity-collection-subregion-path-detail-context-1.0",relationRendering:"edge-routed-clickable-explained-bundled",environmentRendering:"data-derived-static-overview-fade-to-tile-cards",visualTheme:"yujian-shanhai-assets",scriptureDirectory:"eighteen-full-content-pages",waterDisplay:"special-model-audited-dossier-and-render-tiers",bootGuard:true};
  setupV027State();
  init();
