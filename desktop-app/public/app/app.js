...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function v101PreviewValue(value){
    ...
    let text="";try{text=JSON.stringify(value,(key,item)=>typeof item==="string"&&/^data:image\//i.test(item)?`[图片数据，约 ${Math.max(1,Math.round(item.length*0.75/1024))} KB]`:item,2)}catch{text=String(value)}
  ...
  function batchPatchPreviewHtml(session){
    ...
    packages.forEach(item=>(item.simulation?.results||[]).filter(result=>result.status==="conflict").forEach(result=>v101ConflictDetails(result).forEach(detail=>{const key=v101ConflictResolutionKey(result.conflictKey,detail.path),choice=session.conflictResolutions?.[key]||"local";conflictRows.push(`<article class="batch-conflict-item"><header class="batch-conflict-item-head"><div><strong>${esc(result.target)} · ${esc(v101ReadablePath(detail.path))}</strong><span>${esc(item.entry?.name||"更改包")} · ${esc(result.operationLabel)}</span></div><label>采用哪一边<select data-conflict-resolution="${esc(key)}"><option value="local" ${choice==='local'?'selected':''}>保留本机当前内容（推荐）</option><option value="remote" ${choice==='remote'?'selected':''}>替换为更改包内容</option></select></label></header><div class="conflict-value-grid"><section class="conflict-value local"><b>本机当前内容</b><pre>${esc(v101PreviewValue(detail.localValue))}</pre></section><span class="conflict-value-arrow" aria-hidden="true">→</span><section class="conflict-value remote"><b>更改包内容</b><pre>${esc(v101PreviewValue(detail.remoteValue))}</pre></section></div></article>`)})));
    return `<div class="batch-patch-head"><div><span class="eyebrow">BATCH SHJPATCH · v1.0.1</span><h3>批量更改包合并与冲突选择</h3><p>来源：${sourceLabel} · 安全内容照常合并；冲突字段会直接显示修改前后内容。</p></div><button class="btn secondary compact" id="batchPatchChooseAgain">${session.source==='github'?'返回更新列表':'重新选择文件'}</button></div><div class="batch-patch-rules"><span><b>1</b> 不同图片与资料项自动合并</span><span><b>2</b> 左右对照本机与更改包内容</span><span><b>3</b> 每个字段独立选择，默认保留本机</span><span><b>4</b> 桌面端写入前自动建立备份</span></div><div class="batch-selection-tools"><label><input type="checkbox" id="batchPatchSelectAllSafe" ${selected.size&&selected.size===selectable.length?'checked':''}> 全选可处理更改包</label><button class="btn secondary compact" id="batchPatchSelectSafe">恢复推荐选择</button><span>已选 <b>${selected.size}</b>／${selectable.length} 份</span></div><div class="patch-counts"><span class="apply">将写入 ${session.applyCount}项</span><span class="skip">自动合并 ${session.autoMergedCount||0}项</span><span class="conflict">手动裁定 ${session.manualCount||0}项</span></div>${conflictRows.length?`<section class="batch-conflict-panel"><div class="batch-conflict-title"><strong>逐字段处理冲突</strong><span>${conflictRows.length}个字段 · 默认保留本机</span></div>${conflictRows.join("")}</section>`:`<div class="patch-selection-safe"><strong>没有剩余冲突</strong><p>图片图库和不同资料项已按内容自动合并。</p></div>`}<table class="change-table patch-table batch-patch-table"><thead><tr><th>选择／状态</th><th>更改包</th><th>记录</th><th>检查结果</th></tr></thead><tbody>${rows}</tbody></table><div class="pending-preview-footer"><p>${session.blocked?'所选文件本身格式错误，不能应用。':session.actionable.length?'确认后应用安全内容和上面的逐字段选择；未选更改包不会写入。':'尚未选择需要处理的新更改包。'}</p><button class="btn primary" id="batchPatchApplyBtn" ${session.blocked||!session.actionable.length?'disabled':''}>应用已选 ${session.actionable.length} 份更改包</button></div>`
  }
