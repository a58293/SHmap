...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  const state = {
    ...
    objectRevision:1,
    profileRevision:1,
    perf:{indexRevision:-1,objectById:new Map(),cellMap:new Map(),anchorGroups:[],anchorGroupByKey:new Map(),areas:[],lines:[],bounds:null,searchKey:"",searchContext:null,spatialKey:"",spatialContext:null,regionCache:new Map(),isolatedRevision:-1,isolated:[],minimapRevision:-1,minimapSize:"",labelTimer:null,saveTimer:null,saveStatusTimer:null,lightRenderQueued:false,brushDrawQueued:false,wheelBase:null,wheelSettleTimer:null,panLastRebase:0,panRebaseCount:0,relationHitAreas:[],waterHitAreas:[],locationKey:"",locationContext:null},
  ...
  function invalidateObjectCaches(){
    ...
    const p=state.perf;p.indexRevision=-1;p.searchKey="";p.searchContext=null;p.spatialKey="";p.spatialContext=null;p.regionCache.clear();p.isolatedRevision=-1;p.minimapRevision=-1;
  }
  ...
  function renderPrecisionLayer(){
    const v=visibleWorld(),pad=Math.max(120,v.right-v.left,v.top-v.bottom),stateSearch=getSearchContext(),focus=getSpatialFocusContext(),groups=ensureObjectIndexes().anchorGroups;
  ...
  function regionOverviewGroups(){
    const cacheKey=[state.objectRevision,state.filters.q,state.filters.chapter,state.filters.type,state.camera.zoom<=.27?"far":"near"].join("|");if(state.perf.regionCache.has(cacheKey))return state.perf.regionCache.get(cacheKey);
  ...
  function openWaterPath(path){
    if(!path)return;state.selectedWaterPathId=path.id;state.hoverWaterPathId=null;const linked=(path.objectIds||[]).map(indexedObject).find(Boolean);if(linked){state.selectedId=linked.id;const cell=objectCell(linked);state.selectedCell=cellKey(cell.gx,cell.gy)}else if(path.source){state.selectedCell=cellKey(cellIndex(path.source[0]),cellIndex(path.source[1]))}
...
完整的地块或对象描述。`;
  ...
  function openFullDossierForTile(key){document.getElementById("brushCollectionWorkspace")?.classList.add("hidden");state.dossierCollectionMode=false;state.dossierPrecisionGroup=null;state.selectedCell=key;const items=objectsInCellKey(key);if(items[0])state.selectedId=items[0].id;state.dossierMode="full";els.dossierWorkspace.classList.remove("hidden");renderDossierWorkspace()}
  ...
  function openPrecisionDossier(objectId,groupKey){const o=indexedObject(objectId);if(!o)return;const c=objectCell(o);state.spatialFocusArmed=true;state.selectedId=o.id;state.selectedCell=cellKey(c.gx,c.gy);state.precisionClusterOpen=null;state.dossierCollectionMode=false;state.dossierPrecisionGroup=groupKey||`${objectAnchor(o).x.toFixed(1)},${objectAnchor(o).y.toFixed(1)}`;state.dossierCategoryFilter="all";state.dossierMode="brief";state.dossierTab="overview";renderDetails();renderSidebar();scheduleRender();persist();els.dossierWorkspace.classList.remove("hidden");renderDossierWorkspace()}
