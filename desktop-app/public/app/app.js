...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function recordChange(c){const cellEntity=String(c.entityId||"").startsWith("CELL-")||c.entityType==="tile_profile";if(!cellEntity||c.before?.objects||c.after?.objects)invalidateObjectCaches();if(cellEntity)invalidateProfileCaches();state.changes.push({...c,changeId:`CHG-${Date.now()}-${Math.random().toString(36).slice(2,6)}`,time:new Date().toLocaleString("zh-CN"),baseVersion:state.dataVersion});updateHeader()}
...
完整的地块或对象描述。`;
  ...
  function v029RelationDetails(a,b){
    ...
    const ac=objectCell(a),bc=objectCell(b);if(ac.gx===bc.gx&&ac.gy===bc.gy)add("同一地块","context",`同处${tileCoordCode(ac.gx,ac.gy)}`);
    if(a?.region&&b?.region&&normalizeText(a.region)===normalizeText(b.region))add("同属区域","context",a.region);
  ...
  function v044RelationPresetDescription(p=state.viewPreset||"all"){
    return ({
      all:"显示全部直接关系与空间关联",
      geography:"只显示地貌、区域和同地块等空间关系",
      hydrology:"只显示水系源出、流经、汇入、支流与交汇关系",
      myth:"只显示神祇、异兽、神木与神话事件相关关系",
      civilization:"只显示国族、人群、城邑、祭祀与文明事件相关关系",
      evidence:"只显示资料中明确提及的直接关系"
    })[p]||"按当前视图筛选关系";
  }
  function v044RelationPresetMatch(selected,target,info){
    const preset=state.viewPreset||"all";if(preset==="all")return true;
    const reasons=info?.reasons||[],labels=reasons.map(r=>r.label).join(" "),explicit=reasons.some(r=>r.level==="explicit"),sameTile=/同一地块/.test(labels),sameRegion=/同属区域/.test(labels);
    const category=o=>objectCategory(o),text=o=>`${o?.type||""} ${o?.name||""} ${o?.localRelation||""} ${o?.relationNotes||""}`;
    const terrain=o=>category(o)==="terrain"&&!isHydrologyObject(o);
    const myth=o=>category(o)==="plants"||category(o)==="animals"||(category(o)==="people"&&/神祇|神女|神人|巫/.test(objectPrimaryType(o)))||/神话|异怪|灵物|神迹|神域/.test(text(o));
    const civilization=o=>category(o)==="people"||category(o)==="events"||/国|民|人|帝|王|宫|城|邑|部|族|祭|祀|聚落|都城/.test(text(o));
    if(preset==="hydrology"){
      const waterRelation=/源出|作为源头|流经|被其流经|汇入|接纳汇入|支流|交汇/.test(labels);
      return waterRelation||((isHydrologyObject(selected)||isHydrologyObject(target))&&explicit);
    }
    if(preset==="geography")return (terrain(selected)||terrain(target))&&(explicit||sameTile||sameRegion);
    if(preset==="myth")return ((myth(selected)||myth(target))&&explicit)||(myth(selected)&&myth(target)&&(sameTile||sameRegion));
    if(preset==="civilization")return ((civilization(selected)||civilization(target))&&explicit)||(civilization(selected)&&civilization(target)&&(sameTile||sameRegion));
    if(preset==="evidence")return explicit;
    return true;
  }
  function v029RelationContext(){
    const selected=state.objects.find(o=>o.id===state.selectedId);if(!state.relationMode||!selected)return {selected:null,ids:new Set(),strong:new Set(),scored:[]};
    const preset=state.viewPreset||"all",limit=preset==="all"?28:preset==="evidence"?24:18;
    const scored=state.objects.map(o=>({o,score:v029RelationScore(selected,o),dist:Math.hypot((o.x||0)-(selected.x||0),(o.y||0)-(selected.y||0))})).filter(x=>x.score>0).map(x=>({...x,info:v029RelationDetails(selected,x.o)})).filter(x=>v044RelationPresetMatch(selected,x.o,x.info)).sort((a,b)=>b.score-a.score||a.dist-b.dist).slice(0,limit);
    return {selected,ids:new Set(scored.map(x=>x.o.id)),strong:new Set(scored.filter(x=>x.info.level==="explicit"||x.score>=7).map(x=>x.o.id)),scored}
  }
  ...
  function showV044RelationTooltip(hit,x,y){const target=indexedObject(hit.targetId),selected=indexedObject(hit.selectedId);if(!target||!selected)return;els.tooltip.className="map-tooltip relation-line-tooltip";els.tooltip.innerHTML=`<strong>${esc(hit.info.label)} · ${esc(target.name)}</strong><small>${esc(selected.name)} → ${esc(target.name)}</small><p>${esc(hit.info.detail||"当前对象与目标对象存在资料或空间关系。")}</p><em>单击关系线，跳转到线另一端</em>`;els.tooltip.classList.remove("hidden");moveTooltip(x,y)}
  ...
  function updateV029ToolUI(){
    ...
    const relationContext=state.relationMode?v029RelationContext():null,relationPresetLabel=V029_PRESETS[state.viewPreset]||"全部",legendHint=document.getElementById("relationLegendHint");legend?.classList.toggle("hidden",!state.relationMode);if(legendMeta)legendMeta.textContent=relationContext?.selected?`${relationContext.selected.name} · ${relationPresetLabel}视图 · ${relationContext.scored.length}条关系`:"请先选择一个对象";if(legendHint)legendHint.textContent=`${v044RelationPresetDescription()}；悬停查看，单击跳转`;if(!state.relationMode){state.perf.relationHitAreas=[];state.relationHoverId=null;els.viewport.classList.remove("relation-line-hover");if(els.tooltip.classList.contains("relation-line-tooltip"))hideTooltip()}
