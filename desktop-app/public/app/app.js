...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function showChanges(){
    const rows=state.changes.length?state.changes.slice().reverse().map(c=>`<tr data-change-row="${esc(c.changeId||'')}"><td>${esc(c.operationLabel)}</td><td>${esc(c.after?.name||c.before?.name||c.entityId)}</td><td>${esc(c.summary)}</td><td>${esc(c.time)}</td><td class="change-row-actions"><button type="button" class="btn secondary compact" data-change-edit="${esc(c.changeId||'')}">${c.operation==='delete'?'恢复后修改':'修改'}</button></td></tr>`).join(""):`<tr><td colspan="5">本轮尚无更改。</td></tr>`;
    ...
    els.infoEyebrow.textContent="LOCAL CHANGE LOG";els.infoTitle.textContent=`更改记录 · 本轮 ${state.changes.length} 项`;els.infoBody.innerHTML=`<div class="change-actions"><button class="btn secondary compact" id="changesExportNow" ${state.changes.length?"":"disabled"}>仅导出本轮</button><button class="btn primary compact" id="changesFinishRound" ${state.changes.length?"":"disabled"}>完成本轮编辑</button><button class="btn compact changes-discard-round" id="changesDiscardRound" ${state.changes.length?"":"disabled"}>删除本轮修改</button></div><div class="info-section"><h3>本轮自动简述</h3><p>${esc(buildChangeSummary())}</p></div><table class="change-table"><thead><tr><th>操作</th><th>对象</th><th>内容</th><th>时间</th><th>处理</th></tr></thead><tbody>${rows}</tbody></table><div class="change-section-title"><h3>历史提交</h3><span>${state.changeArchives.length}轮</span></div>${archives}<div class="change-section-title"><h3>已应用外部更改包</h3><span>${state.remotePatchHistory.length}个</span></div>${remote}`;
    openModal("infoModal");const exportBtn=$("#changesExportNow"),finishBtn=$("#changesFinishRound"),discardBtn=$("#changesDiscardRound");if(exportBtn)exportBtn.addEventListener("click",()=>exportPatch(false));if(finishBtn)finishBtn.addEventListener("click",()=>{closeModal("infoModal");finishRoundAndPublish()});if(discardBtn)discardBtn.addEventListener("click",v050DiscardRoundChanges);els.infoBody.querySelectorAll("[data-change-edit]").forEach(button=>button.addEventListener("click",()=>v115OpenChangeEditor(button.dataset.changeEdit)));els.infoBody.querySelectorAll("[data-archive-uploaded]").forEach(b=>b.addEventListener("click",()=>markArchiveUploaded(b.dataset.archiveUploaded)))
  }
