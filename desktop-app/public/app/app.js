...
完整的地块或对象描述。`;
  ...
  function v050OrganizeControls(){
    ...
    const actions=document.querySelector(".top-actions");if(!actions)return;let data=actions.querySelector(".v050-top-data"),system=actions.querySelector(".v050-top-system");if(!data){data=document.createElement("div");data.className="v050-top-data";["openImportBtn","checkUpdateBtn","exportPatchBtn"].forEach(id=>{const el=document.getElementById(id);if(el)data.appendChild(el)});actions.prepend(data)}if(!system){system=document.createElement("details");system.className="v050-top-system";system.innerHTML='<summary>系统</summary><div></div>';actions.insertBefore(system,document.getElementById("finishRoundBtn"))}const menu=system.querySelector("div");["desktopUpdateBtn","desktopBackupBtn"].forEach(id=>{const el=document.getElementById(id);if(el&&!menu.contains(el))menu.appendChild(el)});v050InjectTextScaleControl()
  }
  ...
  window.__SHJ_APP_RUNTIME_INFO__={version:"0.9.5",renderArchitecture:"single-static-runtime",objectRoleSchema:"entity-collection-subregion-path-detail-context-1.0",relationRendering:"edge-routed-clickable-explained-bundled",environmentRendering:"data-derived-static-overview-fade-to-tile-cards",visualTheme:"yujian-shanhai-assets",scriptureDirectory:"eighteen-full-content-pages",waterDisplay:"special-model-audited-dossier-and-render-tiers",imageSync:"adaptive-webp-sha256-github-assets",tileImageMap:"switchable-image-or-terrain-card",bootGuard:true};
  setupV027State();
  init();
