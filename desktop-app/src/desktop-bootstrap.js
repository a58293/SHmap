...
async function checkDesktopUpdate({silent=false}={}){
  ...
  try{
    ...
    if(info){
      renderUpdateState(info,`发现新版本 v${info.version}`);
    ...
    if(state)state.textContent=message.includes("404")?"更新服务器尚未发布版本":"检查失败，请确认网络连接";
    updateProgress(0,"未完成");
