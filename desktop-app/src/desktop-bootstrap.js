...
function setupNativeUi(){
  ...
  const openUpdate=async()=>{updateModal.classList.remove("hidden");try{const info=await invoke("app_version");document.querySelector("#desktopCurrentVersion").textContent=`${info.edition} · ${info.version}`;}catch{document.querySelector("#desktopCurrentVersion").textContent="v005";}};
