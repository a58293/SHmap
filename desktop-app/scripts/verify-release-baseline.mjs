...
assert.equal(pkg.version,"1.2.5");assert.equal(version.semver,"1.2.5");assert.equal(tauri.version,"1.2.5");assert.equal(cv,"1.2.5");assert.equal(version.data_version,"v283-r0001");assert.equal(version.object_count,1378);assert.equal(version.board_occupied_cells,875);assert.equal(version.water_arrow_cells,48);assert.equal(release.bundle?.createUpdaterArtifacts,true);
...
console.log("PASS v1.2.5 / V283 release baseline");
