...
assert.equal(pkg.version,"1.1.2");assert.equal(version.semver,"1.1.2");assert.equal(tauri.version,"1.1.2");assert.equal(cv,"1.1.2");assert.equal(version.data_version,"v272-r0003");assert.equal(version.object_count,624);assert.equal(version.board_occupied_cells,497);assert.equal(release.bundle?.createUpdaterArtifacts,true);
...
console.log("PASS v1.1.2 region / people-civilization / stable-ui fixes");
