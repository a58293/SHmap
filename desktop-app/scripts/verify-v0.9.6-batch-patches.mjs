...
for (const path of ["public/app/app.js", "dist/app/app.js"]) {
  ...
  assert.ok(app.includes('batchPatch:"content-only-preview-with-multi-select-resolution"'), `${path} 运行信息未声明v1.0.4内容预览与多选裁定能力`);
}
...
console.log("v1.0.4 批量更改包专项校验通过：只比较实际地块内容，支持冲突字段多选处理，应用前备份与失败回退均保留。");
