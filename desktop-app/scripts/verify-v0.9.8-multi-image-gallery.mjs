...
const hasLegacyGitTimeout =
  rustSource.includes('GCM_INTERACTIVE", "never"') &&
  rustSource.includes('http.lowSpeedTime=30');

const hasPrivateApiTimeout =
  rustSource.includes("github_auth::publish_private_submission") &&
  rustSource.includes("timeout(") &&
  rustSource.includes("Duration::from_secs(180)");
...
checks.push(
  ...
  [hasLegacyGitTimeout || hasPrivateApiTimeout, "GitHub上传不会无限等待慢或失速网络"],
