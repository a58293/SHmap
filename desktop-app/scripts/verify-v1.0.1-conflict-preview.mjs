...
for(const path of ["public/app/app.js","dist/app/app.js"]){
  const app=read(path);
  for(const marker of ["function v101ReadablePath","function v101PreviewValue","function v101ConflictResolutionKey","本机当前内容","更改包内容","逐字段处理冲突","conflict-value-grid"])assert.ok(app.includes(marker),`${path} 缺少冲突预览标记：${marker}`);
}
