...
SOURCE = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else ROOT / "v283.xlsx"
...
OUTPUT = Path(sys.argv[3]).resolve() if len(sys.argv) > 3 else ROOT / "SHmap-Data-v283"
...
for row in range(9, 1387):
    ...
    name = raw_name
    # V283 has already regenerated the display layer from the formal-name
    # column. Historical display notes in the variants field must not
    # override the V283 formal name or its board-cell label.
    if not name:
        continue
    row_ref = f"R{row}"
    ...
    if object_id in used_ids:
        object_id = f"SHJ-OBJ-V283-{row:06d}"
    ...
    placement = {
        "sourceSheet": "坐标棋盘图", "sourceWorkbookVersion": "V283", "cell": address,
    ...
    obj = {
        ...
        "primaryRegionId": layer_id, "regionIds": [layer_id], "macroRegionId": layer_id, "macroAssignment": "excel-v283",
        "boardPlacement": placement,
        "imageUrl": normalize((old or {}).get("imageUrl")),
    }
    if old:
        for key in [
            "images", "imageSource", "imageCopyright", "dossier", "childHierarchy", "waterHierarchy", "notesLocal",
            "terrain", "water", "plants", "animals", "minerals", "wildlife", "beasts", "people", "gods", "residents",
            "appearance", "abilities", "events", "annotations", "otherTexts", "modernResearch", "commonLocation",
...
for address, raw_text in sorted(board.items(), key=lambda pair: (row_index(pair[0]), col_index(pair[0]))):
    ...
    water_flow_annotations.append({
        ...
        "hasFormalObject": False, "sourceWorkbookVersion": "V283",
    })

if len(water_flow_annotations) != 48:
    raise RuntimeError(f"V283应有48个非对象水系箭头格，实际识别{len(water_flow_annotations)}个")
...
for layer_id, label in layer_names.items():
    ...
    hierarchy_regions.append({
        ...
        "status": "excel-v283-authoritative", "source": "V283 世界层级裁决",
    })
...
# V283 replaces the old general-purpose overlay with 48 exact board cells.
# Retain the reviewed legacy topology for dossiers and audit history, but mark
# it as non-default so it cannot reintroduce the display residue V283 removed.
water_paths = old_globals["SHJ_WATER_PATHS"]
for path in water_paths:
    path["overlaySourceVersion"] = "V272"
    path["officialObjectPlacementSource"] = "V283"
    path["legacyOverlayHidden"] = True
    path["migrationNote"] = "V283默认只显示电子表格确认的48个河名＋箭头格；旧拓扑仅保留作档案与专项水系研究参考"
board_layout = {
    "schemaVersion": "v283-board-layout-1", "sourceWorkbook": SOURCE.name, "sourceSheet": "坐标棋盘图",
    ...
    "rules": [
        "R9—R1386正式对象必须按V283表2格位实装", "都广之野固定为唯一全局原点(0,0)",
        "L1中央大陆百里链继续按1格=100里", "四海宽度、海外近海和大荒外围轮廓不得反推为原典实距",
        "关系锁定对象新增冲突前不得单点移动", "红色推定对象只能按V283允许调整方式移动；山系组与附着对象不得拆散",
    ...
    "annotationCount": len(water_flow_annotations), "arrowAnnotationCount": len(water_flow_annotations),
}

generated_at = dt.datetime.now(dt.timezone.utc).isoformat().replace("+00:00", "Z")
metadata = {
    "appName": "山海经原典地图研究台 桌面版", "dataVersion": "v283-r0001", "schemaVersion": "desktop-v283-display-cleanup",
    "specResearch": "v283", "specProduction": "v283", "origin": {"name": "都广之野", "x": 0, "y": 0},
    "mainGridLi": 100, "innerGridLi": 10, "objectCount": len(objects), "paragraphEntryCount": len(original_library),
    "waterPathCount": len(water_paths), "waterArrowCellCount": len(water_flow_annotations), "generatedAt": generated_at, "sourceWorkbook": SOURCE_DISPLAY_NAME,
...
hierarchy = {
    "schemaVersion": "world-region-place-v283", "generatedAt": generated_at,
    "world": {"id": "world-shanhaijing", "name": "山海经世界", "type": "world", "origin": {"name": "都广之野", "x": 0, "y": 0},
              ...
              "bounds": {"minX": -16000, "maxX": 14000, "minY": -13600, "maxY": 14000}, "objectCount": len(objects), "status": "v283-authoritative"},
...
spec = {
    "precedence": "V283电子表格优先于V282、V272和客户端旧字段",
    "rules": [
        ...
        ["显示", "875个对象格只显示原典正式名称与必要方向／水流关系；不显示R号、工程码、接口、走廊或纯箭头"],
...
bundle = {"format": "shmap-private-bootstrap-v1", "dataVersion": "v283-r0001", "globals": globals_}
payload = json.dumps(bundle, ensure_ascii=False, separators=(",", ":"))
digest = hashlib.sha256(payload.encode("utf-8")).hexdigest()
manifest = {
    "schemaVersion": "shmap-private-data-manifest-v1", "dataVersion": "v283-r0001", "dataPath": "production/map-data.json",
    "sha256": digest, "objectCount": len(objects), "paragraphEntryCount": len(original_library), "waterPathCount": len(water_paths),
    "generatedAt": generated_at, "sourceFile": SOURCE_DISPLAY_NAME, "minimumDesktopVersion": "1.2.3",
    "boardLayoutSchema": "v283-board-layout-1", "boardPlacementCount": len(objects), "boardOccupiedCellCount": len(occupied_cells),
    "waterArrowCellCount": len(water_flow_annotations),
    "worldLayerCounts": metadata["worldLayerCounts"], "coordinateStatusCounts": metadata["coordinateStatusCounts"],
    "migrationNote": "V283电子表格为正式地图权威来源；保持V282坐标不动，迁移仅保留本地博物志与图片，显示层只采用48个权威水系箭头格。",
}
...
(ROOT / "v283-build-report.json").write_text(json.dumps({
