...
同名对象：不会自动覆盖；程序会提示可能重复。`;
  ...
  const els = {
    versionLine:$("#versionLine"), saveState:$("#saveState"), topChangeCount:$("#topChangeCount"), topTrashCount:$("#topTrashCount"), openTrashTab:$("#openTrashTab"),
    ...
    deleteModal:$("#deleteModal"), deleteModalMeta:$("#deleteModalMeta"), deleteChoices:$("#deleteChoices"), trashModal:$("#trashModal"), trashList:$("#trashList"),
  ...
  function persist(){
    try{localStorage.setItem(STORAGE_KEY, JSON.stringify({objects:state.objects,changes:state.changes,dataVersion:state.dataVersion,camera:state.camera,selectedId:state.selectedId,selectedCell:state.selectedCell,tileProfiles:state.tileProfiles,trash:state.trash})); els.saveState.textContent="已保存到本地"; setTimeout(()=>els.saveState.textContent="本地工作区",900)}catch(e){console.warn(e)}
  }
  ...
  function init(){
    populateFilters(); bindEvents(); renderSidebar(); renderDetails(); resizeCanvas(); scheduleRender(); updateHeader(); renderExamplePanel();
  ...
  function nextObjectId(){let max=0;state.objects.forEach(o=>{const m=String(o.id).match(/(\d+)$/);if(m)max=Math.max(max,Number(m[1]))});return `SHJ-OBJ-${String(max+1).padStart(6,"0")}`}
  ...
  function pushTrash(item){state.trash.unshift({trashId:`TRASH-${Date.now()}-${Math.random().toString(36).slice(2,6)}`,deletedAt:new Date().toLocaleString("zh-CN"),...item})}
  ...
  function exportPatch(){if(!state.changes.length){toast("没有可导出的更改","先新增或编辑对象后再导出。","error");return}const now=new Date(),stamp=now.toISOString().replace(/[-:]/g,"").replace(/\..+/,"");const pkg={package_type:"shjpatch",package_version:"1.0",base_data_version:state.dataVersion,created_at:now.toISOString(),change_count:state.changes.length,summary:buildChangeSummary(),changes:state.changes};download(`山海经地图更改包_${stamp}.shjpatch`,JSON.stringify(pkg,null,2));toast("更改包已导出",`${state.changes.length}项更改，可上传到GitHub仓库。`)}
