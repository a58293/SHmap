...
同名对象：不会自动覆盖；程序会提示可能重复。`;
  ...
  function bindTileEvents(){
    els.tileLayer.querySelectorAll(".tile").forEach(tile=>{
      tile.addEventListener("click",e=>{if(Date.now()<state.suppressClickUntil)return;if(e.target.closest("button"))return;if(clickOutsideSpatialFocus(tile.dataset.cell))return;state.selectedCell=tile.dataset.cell;state.flippedCell=state.flippedCell===tile.dataset.cell?null:tile.dataset.cell;renderDetails();scheduleRender();persist()});
