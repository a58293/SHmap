...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function renderMap(){
    ...
    drawCanvas(); renderTiles(); drawBrushTraceCanvas();
  ...
  function persist(){
    try{
      localStorage.setItem(STORAGE_KEY,JSON.stringify({
        ...
        dossierMode:state.dossierMode,brushKeys:[...(state.brushKeys||[])],brushStrokes:state.brushStrokes||[],viewPreset:state.viewPreset,compareKeys:[...(state.compareKeys||[])]
      }));
  ...
  function startBrushTrace(clientX,clientY){
    ...
    if(state.brushStrokes.length>60)state.brushStrokes.splice(0,state.brushStrokes.length-60);drawBrushTraceCanvas()
  }
  function finishBrushTrace(){state.brushCurrentStroke=null;drawBrushTraceCanvas()}
  function clearBrushTraces(){state.brushCurrentStroke=null;state.brushStrokes=[];drawBrushTraceCanvas()}
