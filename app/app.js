...
重叠对象：多个对象使用相同精确坐标，程序自动用“/”显示全部名称。`;
  ...
  function renderPrecisionLayer(){
    const v=visibleWorld(),pad=35,stateSearch=getSearchContext(),focus=getSpatialFocusContext(),groups=ensureObjectIndexes().anchorGroups;
  ...
  function flushPersist(){
    clearTimeout(state.perf.saveTimer);state.perf.saveTimer=null;try{localStorage.setItem(STORAGE_KEY,JSON.stringify(workspaceSnapshot()));if(els.saveState){els.saveState.textContent="已保存到本地";clearTimeout(state.perf.saveStatusTimer);state.perf.saveStatusTimer=setTimeout(()=>els.saveState.textContent="本地工作区",900)}}catch(e){console.warn(e)}
  }
  ...
  function scheduleV037PrecisionLabelLayout(){clearTimeout(state.perf.labelTimer);if(!state.precisionMode)return;state.perf.labelTimer=setTimeout(()=>{if(state.pan.active||state.cameraAnimation)return scheduleV037PrecisionLabelLayout();requestAnimationFrame(layoutV029PrecisionLabels)},140)}
