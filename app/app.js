...
同名对象：不会自动覆盖；程序会提示可能重复。`;
  ...
  function tileHTML(gx,gy,k,all,visible,left,top,size,semantic,isChanged,searchClass="",search=null){
    ...
    const frontTitle=showFrontName?`<div class="tile-title-block"><h3>${q?markSearchText(main?.name||"",q):esc(main?.name||"未命名地块")}</h3></div>`:"";
    return `<div class="tile ${flipped?'flipped':''} ${selected?'selected':''} ${isChanged?'changed':''} ${state.previewCell===k?'search-preview':''} ${semantic} ${searchClass} ${pulse}" data-cell="${k}" data-gx="${gx}" data-gy="${gy}" style="${style}"><div class="tile-card"><div class="tile-face tile-front">${changeBadge}<div class="tile-content ${frontMode}"><div class="tile-icon-row"><i class="type-icon featured ${esc(main?.geometryType||'point')}">${geometryIcon(main||all[0])}</i>${all.length>1?`<span class="tile-count">${all.length}</span>`:""}</div>${frontTitle}</div></div><div class="tile-face tile-back"><div class="tile-back-head"><span>${coord}</span><strong>${all.length}项</strong></div><div class="tile-object-stack">${list}</div><div class="tile-actions"><button data-action="drill">10里格内</button><button class="add" data-action="add">＋新增</button><button class="delete" data-action="delete">⌫删除</button></div></div></div></div>`
  }
  ...
  function fieldGlyph(title){const map={"地理环境":"山","建筑群":"邑","生活物种":"生","所属国度":"国","信仰对象":"祀","统治者":"冠","守护神":"守","奇珍异兽／栖息生物":"兽","神木／神话植被":"木","仙草药草":"草","丰富矿产":"矿","特殊生命":"灵","当地风俗":"俗","文明与神话奇遇":"奇","已发生事件（综合简述）":"事","其他剧情说明":"叙","玩家遭遇的敌人":"敌","玩家触发的剧情":"戏","玩家得到的东西":"获"};return map[title]||"录"}
  ...
  function renderDetailBody(profile,items){let html=objectRibbon(items);if(state.detailTab==="summary")html+=`<div class="dossier-section"><div class="dossier-section-head"><h3>地块简述</h3><span>环境 · 建筑 · 物种</span></div>${dossierField("地理环境",profile.geoEnvironment)}${dossierField("建筑群",profile.architecture)}${dossierField("生活物种",profile.livingSpecies)}</div>`;if(state.detailTab==="civilization")html+=`<div class="dossier-section"><div class="dossier-section-head"><h3>文明部落</h3><span>文明结构与资源生态</span></div><div class="dossier-grid">${dossierField("所属国度",profile.country)}${dossierField("信仰对象",profile.faith)}${dossierField("统治者",profile.ruler)}${dossierField("守护神",profile.guardian)}</div>${dossierField("奇珍异兽／栖息生物",profile.beasts)}<div class="dossier-grid">${dossierField("神木／神话植被",profile.divinePlants)}${dossierField("仙草药草",profile.herbs)}${dossierField("丰富矿产",profile.minerals)}${dossierField("特殊生命",profile.specialLife)}</div>${dossierField("当地风俗",profile.customs)}</div>`;if(state.detailTab==="encounters")html+=`<div class="dossier-section"><div class="dossier-section-head"><h3>文明与神话奇遇</h3><span>神迹 · 祭祀 · 文明异象 · 特殊空间</span></div>${dossierField("文明与神话奇遇",profile.mythicEncounters)}</div>`;if(state.detailTab==="story"){const reachable=profile.playerReachable==="yes"||profile.playerReachable==="conditional";html+=`<div class="dossier-section"><div class="dossier-section-head"><h3>剧情</h3><span>玩家层与原典事件分离</span></div><div class="status-line"><span class="status-pill ${statusClass(profile.timeNormal)}">${statusText(profile.timeNormal,"time")}</span><span class="status-pill ${statusClass(profile.playerReachable)}">${statusText(profile.playerReachable,"player")}</span></div>${dossierField("已发生事件（综合简述）",profile.occurredEvents)}${dossierField("其他剧情说明",profile.storyOther)}${reachable?`<div class="player-story-box"><h3>玩家抵达后的内容</h3>${dossierField("玩家遭遇的敌人",profile.playerEnemies)}${dossierField("玩家触发的剧情",profile.playerPlots)}${dossierField("玩家得到的东西",profile.playerLoot)}</div>`:`<div class="player-story-box disabled"><h3>玩家内容暂不展开</h3><div>只有“可以抵达”或“满足条件后可抵达”时，才显示敌人、触发剧情与所得物品。</div></div>`}<div class="dossier-section-head"><h3>十八经篇事件归档</h3><span>非玩家事件</span></div>${scriptureGroupsHTML(profile)}</div>`}els.detailBody.innerHTML=html;els.detailBody.querySelectorAll('[data-tile-object]').forEach(btn=>{btn.addEventListener('click',()=>{state.spatialFocusArmed=true;state.selectedId=btn.dataset.tileObject;renderDetails();renderSidebar();scheduleRender();persist()});btn.addEventListener('dblclick',()=>{const o=state.objects.find(x=>x.id===btn.dataset.tileObject);if(o)openObjectForm(o)})})}
  ...
  function dossierIcon(label){const map={"地形地貌":"山","地域特征":"域","所属气候":"风","主要山脉":"岳","水系河流":"水","建筑群":"邑","生活物种":"生","所属国度":"国","信仰对象":"祀","统治者":"冠","守护神":"守","奇珍异兽":"兽","神木植被":"木","仙草药草":"草","丰富矿产":"矿","特殊生命":"灵","当地风俗":"俗","文明与神话奇遇":"奇","事件":"事","时间":"时","可达性":"行","敌人":"敌","触发剧情":"戏","所得物品":"获"};return map[label]||"录"}
  ...
  function renderBriefDossier(profile,items,tile,main,b){
    ...
    const storyRows=[briefRow("已发生事件",profile.occurredEvents),profile.timeNormal&&profile.timeNormal!=="unknown"?briefRow("时间流逝",statusText(profile.timeNormal,"time")):"",briefRow("其他剧情说明",profile.storyOther),profile.playerReachable&&profile.playerReachable!=="unknown"?briefRow("玩家可达性",statusText(profile.playerReachable,"player")):"",reachable?briefRow("玩家遭遇的敌人",profile.playerEnemies):"",reachable?briefRow("玩家触发的剧情",profile.playerPlots):"",reachable?briefRow("玩家得到的东西",profile.playerLoot):""];
    const story=briefSection("剧情",storyRows);
